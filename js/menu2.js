...
// Obtener relación a los contenedores de las tarjetas y del los dropdown-item
const container = document.querySelector(".here");
const dropContainer = document.querySelector(".btn-container");
...
//se guardan las estructuras de las tarjetas de los elementos del array
function mostrarProductos(menu) {
  var mostrado='';
  for (let producto of menu){
    mostrado+=`<div class="col-sm-4 " style="height: 400px" >
            <div class="card m-2 product overflow-auto" style="max-height: 90% !important; width: 90%">
              <div class="card-body justify-content-center p-3">
                <h4 class="fs-3 mt-3 text-center fw-semibold" >${producto.title}</h4>
                <div style="display: flex;justify-content: center;">
                  <img class="img" src="${producto.img}" align="center">
                </div>
                <h5 class="fs-4 text-right fw-semibold m-2" style="color: #6411C2">$${producto.price}</h5>
                <p class="fs-6 text-justify">${producto.desc}</p>
                <br>
              </div> 
            </div>
          </div>`;
...
function mostrarCategorias() {
  ...
  for (let categoria of categorias){
    drop_item+=`<li><a class="dropdown-item filtro-drop">${categoria}</a></li>`;
  }
  //se modifica el contenido del container del dropdown
  dropContainer.innerHTML = drop_item;

  //se seleccionan todos los dropdown-item
  const drop_filtro = dropContainer.querySelectorAll(".filtro-drop");
  //para cada dropdown-item
  for(let btn of drop_filtro){
    //si se les da clic
    btn.addEventListener("click", function () {
      //se obtiene el nombre del boton
      var categoria = btn.textContent;
